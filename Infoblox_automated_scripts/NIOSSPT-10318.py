...
class NIOSSPT_10318(unittest.TestCase):
    ...
    def test_001_modify_the_function_add_huge_pages(self):
        ...
        try:
            ...
            child.sendline("scp /infoblox/one/bin/fastpath_control root@"+config.client_vip+":/import/qaddi/API_Automation/WAPI_PyTest/suites/customer_bug_automation/fastpath_control")
    ...
    def test_002_replace_content_to_fastpath(self):
        ...
        with open("/import/qaddi/API_Automation/WAPI_PyTest/suites/customer_bug_automation/fastpath_control","r+") as file:
            #print(file.read())
            with open("/import/qaddi/API_Automation/WAPI_PyTest/suites/customer_bug_automation/fastpath_control_temp", "w") as f:
        ...
        try:
            ...
            child.expect("-bash-4.0#",timeout=100)
            child.sendline("scp root@"+config.client_vip+":/import/qaddi/API_Automation/WAPI_PyTest/suites/customer_bug_automation/fastpath_control_temp /infoblox/one/bin/fastpath_control")
            child.expect('password:',timeout=100)
            child.sendline("infoblox")
            child.expect("-bash-4.0#")
            child.sendline("exit")
    ...
    def test_003_Reboot_the_appliance(self):
        ...
        for i in range(1,15):
            sleep(22)
            args = "sshpass -p 'infoblox' ssh -o StrictHostKeyChecking=no root@"+config.client_vip
