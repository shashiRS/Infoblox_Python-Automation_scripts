...
class NIOSSPT_8743(unittest.TestCase):
    ...
    def test_000_setup(self):
        ...
        '''Add DNS Forwarder'''
        display_msg("Add DNS forwarder : 10.0.2.35")
        ...
        data = client_ip+" 32 N/A N/A 'ACS:Acct-Session-Id=9889732d-34590010;NAS:NAS-PORT=1813;PXY:Proxy-All=1;PCP:Parental-Control-Policy=08041000000196ffff37f7;SSP:Subscriber-Secure-Policy=7fffffff;SUB:Calling-Station-Id=04321012134;PXP:PXY_PRI=0ac4065f;PXS:PXY_SEC=0ac4065f'"
    ...
    def test_002_send_dig_query_validate_log_messages_1(self):
        ...
        display_msg("----------------------------------------------------")
        start_syslog('/var/log/messages')
        display_msg("Send dig query to i.ytimg.com")
        dig_cmd = 'dig @'+config.grid_vip+' i.ytimg.com'
        os.system(dig_cmd)
        sleep(30)
        stop_syslog('/var/log/messages')
        if not validate_syslog('/var/log/messages', '\[A\] via i.ytimg.com'):
    ...
    def test_003_send_dig_query_validate_log_messages_2(self):
        ...
        sleep(30)
        stop_syslog('/var/log/messages')
        if not validate_syslog('/var/log/messages', '\[A\] via jackdaniels.com'):
            display_msg("Failure: Expected log message not found")
            assert False
        display_msg("Expected log message found")
    ...
    def test_cleanup(self):
        ...
        display_msg("Delete DNS forwarder : 10.0.2.35")
        data = {"forwarders":[]}
        response = ib_NIOS.wapi_request('PUT', ref=json.loads(grid_dns_ref)[0]['_ref'], fields=json.dumps(data))
        display_msg(response)
        if type(response) == tuple:
            if response[0]==400 or response[0]==401:
                display_msg("Failure: Deleting DNS forwarder : 10.0.2.35")
        
        display_msg("Delete DNS Resolver : 127.0.01, 10.0.2.35")
        grid_ref = ib_NIOS.wapi_request('GET', object_type='grid')
        ...
        if type(response) == tuple:
            if response[0]==400 or response[0]==401:
                display_msg("Failure: Deleting DNS Resolver : 127.0.01, 10.0.2.35")
