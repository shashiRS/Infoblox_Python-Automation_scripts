...
class NIOSSPT_9714(unittest.TestCase):
    ...
    def test_002_add_a_DTC_Server1(self):
        ...
        data = {"name":"server1","host":config.client_vip1}
    ...
    def test_003_add_a_DTC_Server2(self):
        ...
        data = {"name":"server2","host":config.client_vip2}
        response = ib_NIOS.wapi_request('POST', object_type="dtc:server", fields=json.dumps(data))
        print response
        if type(response) == tuple:
            if response[0]==400 or response[0]==401:
    ...
    def test_009_query_for_member_selected_in_pool(self):
        ...
        for index,value in enumerate(output):
            if 'ANSWER SECTION:' in value:
                ...
                print("--------------------")
                match = re.match("arec.test.com.\s+\d+\s+IN\s+A\s+"+config.client_vip1, output[index+1])
                match1= re.match("arec.test.com.\s+\d+\s+IN\s+A\s+"+config.client_vip2, output[index+1]) 
                if match:
                    server1_found = True
    ...
    def test_010_Both_Server_appears_in_GREEN(self):
        ...
        print(get_ref)
        for ref in json.loads(get_ref):
            if 'server1' in ref['_ref'] and 'GREEN' in ref['health']['availability']:
    ...
    def test_011_Server1_is_up_Server2_is_down(self):
        ...
        data="reboot_system -a poweroff -c "+config.usrname+" -H "+config.client2_vmid
        os.system(data)
        sleep(30)
        ...
        for ref in json.loads(get_ref):
            if 'server2' in ref['_ref']:
                ...
                print("Server2:Color="+ref['health']['availability']+" Server1:Status="+ref['health']['enabled_state'])
    ...
    def test_012_query_for_member_selected_in_pool(self):
        print("Query_for_member_selected_in_pool")
        #for i in {1..10}; do dig @10.35.169.10 arec.test.com a; sleep 1 ; done
        output = os.popen("for i in {1..10}; do dig @"+config.grid_member2_vip+" arec.test.com a; sleep 1 ; done").read()
        output = output.split('\n')
        print(output)
        server1_found= False
        server2_found= False
    ...
    def test_013_cleanup_object(self):
        ...
        data="reboot_system -a poweron -c "+config.usrname+" -H "+config.client2_vmid
        os.system(data)
        sleep(10)
        print("Restart Services")
        grid =  ib_NIOS.wapi_request('GET', object_type="grid", grid_vip=config.grid_vip)
